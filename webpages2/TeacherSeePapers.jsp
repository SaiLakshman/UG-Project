...
<body>
<center>
<form name= "teacherseepaperForm" method= "post" action="/Project1/campusteacher">
...
ArrayList<TeacherBean> t1= (ArrayList<TeacherBean>) request.getAttribute("teacherpaper");
Papers_Course c= (Papers_Course)request.getAttribute("coursename");
int a= 0,b= 1;
int d= c.getDuration() * 2;%>
<h1><u>View Syllabus and Teachers of <%=c.getCourse_name()%> <%=c.getBatch()%> batch.</u></h1>
...
<tr Class= "sai2">
	...
	{%>
		...
		%>
					...
					<td Class= "sai4"><%=pap.get(cnt).getPaper_title()%>&nbsp;</td>
				<%if(t1.get(cnt).getTeachername() != null){%>
					<td Class= "sai2"><%=t1.get(cnt).getTeachername()%>&nbsp;</td></tr>
				<%}%>
		<%
...
<td align="center"><input type= "button" name= "coursepaper" value="Modify" onclick= "javascript:modifyteacher()"></td></tr>
